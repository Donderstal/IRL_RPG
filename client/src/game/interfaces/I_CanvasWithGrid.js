...
class I_CanvasWithGrid {
    ...
    getTileAtCell( column, row ) {
        if ( column < 1 ) {
            return { "x": -GRID_BLOCK_PX, "y": row * GRID_BLOCK_PX, "col": column, "row": row, direction: FACING_LEFT }
        }
        else if ( row < 1 ) {
            return { "x": column * GRID_BLOCK_PX, "y": -(GRID_BLOCK_PX * 2), "col": column, "row": row, direction: FACING_UP }
        }
        else if ( column > this.grid.cols ) {
            return { "x": this.grid.width + GRID_BLOCK_PX, "y": row * GRID_BLOCK_PX, "col": column, "row": row, direction: FACING_RIGHT }
        }
        else if ( row > this.grid.rows ) { 
            return { "x": column * GRID_BLOCK_PX, "y": this.grid.height + (GRID_BLOCK_PX * 2), "col": column, "row": row, direction: FACING_DOWN }
        }
